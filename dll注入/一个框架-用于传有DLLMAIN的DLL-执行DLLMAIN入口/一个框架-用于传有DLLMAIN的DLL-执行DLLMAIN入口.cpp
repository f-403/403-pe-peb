...
//用于修复PE文件并返回修复后的地址
DWORD WINAPI check_dll(void*);
_forceinline int my_ascii_cmp(const char* sour, const  char* desc);//用于对比字符串
...
int main()
{
    ...
    BYTE* f_sources = new BYTE[f_size]{};
    ReadFile(f, f_sources, f_size, NULL, 0);//读进数组
    LPVOID mem = VirtualAlloc(NULL, f_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    memcpy_s(mem, f_size, f_sources, f_size);
    //读取PE结构并转换RVA为文件偏移
    IMAGE_DOS_HEADER* dos = (IMAGE_DOS_HEADER*)mem;
    IMAGE_NT_HEADERS* nt = (IMAGE_NT_HEADERS*)((char*)mem + dos->e_lfanew);
    ...
    IMAGE_EXPORT_DIRECTORY* my_export = (IMAGE_EXPORT_DIRECTORY*)(mem_to_file(se_head, nt, data.VirtualAddress) + (char*)mem);
    printf("当前DLL带名字的导出函数个数是:%d\n", my_export->NumberOfNames);


    printf("当前dll的名字是:%s\n", mem_to_file(se_head, nt, my_export->Name) + (char*)mem);
    //打印导出表
    DWORD file_post = 0;
    for (int i = 0; i < my_export->NumberOfNames; i++) {
        DWORD* address_name = (DWORD*)(mem_to_file(se_head, nt, my_export->AddressOfNames) + (char*)mem);//获取数组
        printf("%d 函数是:%s\n", i, (mem_to_file(se_head, nt, address_name[i]) + (char*)mem));
        //获取函数序号数组
        WORD* ordinals_name = (WORD*)(mem_to_file(se_head, nt, my_export->AddressOfNameOrdinals) + (char*)mem);
        DWORD ordinals = ordinals_name[i];
        //获取函数地址数组
        printf("%d 函数的序号为:%d\n", i, ordinals);

        DWORD* address_function = (DWORD*)(mem_to_file(se_head, nt, my_export->AddressOfFunctions) + (char*)mem);
        void* function = mem_to_file(se_head, nt, address_function[ordinals]) + (char*)mem;
        printf("当前函数的地址为:%p\n", function);
        file_post = mem_to_file(se_head, nt, address_function[ordinals]);

        std::cout << "函数地址的文件偏移为:" << file_post << std::endl;

    }

    printf("mem地址：%p\n", mem);
    printf("dll函数地址: %p\n", (char*)mem + file_post);
    void* eop = (char*)mem + file_post;
    //((DWORD(*)(void*))eop)  (mem);

    HANDLE p = OpenProcess(PROCESS_ALL_ACCESS, FALSE,18948);
    void* process_mem = VirtualAllocEx(p, NULL, f_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);//在进程中开空间
    WriteProcessMemory(p, process_mem, f_sources, f_size, NULL);//写数据进去
    CreateRemoteThread(p, NULL, 0, LPTHREAD_START_ROUTINE((char*)process_mem+file_post), process_mem, 0, NULL);
...
DWORD check_dll(void* dll_address_) {
    ...
    while (current_list != end_list) {
        ...
        //如果提前找到， 退出
        if (myLoadLibraryA_addr && myGetProcAddress_addr && myVirtualAlloc_addr && mymemset_addr && mymemcpy_s_addr) {
...
_forceinline int my_ascii_cmp(const char* sour, const  char* desc) { //区分大小写
    int sour_len = 0;
